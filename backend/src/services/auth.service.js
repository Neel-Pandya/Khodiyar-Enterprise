...
class AuthService {
  ...
  /**
   * Forgot Password - Initiate the process
  ...
  async forgotPassword(email) {
    try {
      ...
      return {
        ...
        token, // Frontend needs this token to verify OTP
      };
    } catch (error) {
      if (error instanceof ApiError) throw error;
      logger.error('Failed to initiate forgot password', {
        error: error.message,
      });
      throw new ApiError(500, 'Failed to initiate forgot password');
    }
  }
  ...
  async verifyResetOTP(token, otp) {
    try {
      ...
      if (error instanceof ApiError) throw error;
      logger.error('Failed to verify reset OTP', { error: error.message });
      throw new ApiError(500, 'Verification failed');
    }
  }
